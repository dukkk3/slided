...
export const BackgroundSequence: React.FC = () => {
		...
		() =>
			when(
				() =>
					...
					iterationsControls.store.progress === 0,
				() => updateNeededFrame(ITERATIONS[0])
			),
		[iterationsControls.store.progress, promoStore, updateNeededFrame]
	);
	...
	useEffect(() => {
		if (promoStore.sequenceOpeningAnimationEnded) {
			canvasSequence.setCurrentFrame(ITERATIONS[0]);
			canvasSequence.drawCurrentFrame(true);
		}
	}, [canvasSequence, promoStore]);
	...
	return (
		<S.BackgroundFrame>
			{ITERATIONS.map((_, index) => (
				<Iteration key={index} iterations={index} switchVisibility={false}>
					{([iteration]) => (
						<a.div
							style={{
								opacity:
